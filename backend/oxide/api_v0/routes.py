...
@api_v0_bp.route("/flowers/<int:id>", methods=["GET"])
def get_flower_detail(id):
    user = Flower.get(id)
    return flower_schema.dump(user)


@api_v0_bp.route("/flowers", methods=["GET"])
def get_flowers():
...
@api_v0_bp.route("/flowers", methods=["POST"])
def create_flower():
...
@api_v0_bp.route("/flowers/<int:id>", methods=["PUT"])
